...
クエリパラメータ:
- `limit` — 件数上限（デフォルト 100）
- `status` — フィルタ（例: `sent`，`failed`，`deduplicated`）
- `channel` — フィルタ（例: `ntfy`，`none`）
...
`./scripts/backup.sh` はバックアップ作成後，Notify Hub に `source=backup` のイベントを送信する．

- 成功時: `event_type=backup_success`，`severity=info`
- 失敗時: `event_type=backup_failed`，`severity=error`

初期設定の `NOTIFY_SEVERITIES=critical,error` では，成功イベントは履歴に保存されるだけで，失敗イベントは ntfy 通知対象になる．
Notify Hub が停止している場合や `NOTIFY_API_TOKEN` が未設定の場合でも，バックアップ処理自体の終了コードは通知送信に引きずられない．
...
### NOTIFY_HTTP_TIMEOUT_SECONDS が不正な場合

`NOTIFY_HTTP_TIMEOUT_SECONDS` が数値ではない場合，または 0 以下の場合は，warning ログを出して既定値の 5 秒を使用する．
この設定ミスだけでイベント保存が失敗することはない．
