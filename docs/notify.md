...
※DB内には将来の通知送信・設定保存用として `notifications` や `settings` といった予約テーブルが存在しますが，現段階では「イベント記録基盤」としてのみ機能するため未使用である．
...
Uptime Kuma で監視するためのヘルスチェックである．
...
> **詳細設定手順**: 初心者向けの詳しい設定ステップは，[Uptime Kuma Webhook連携 設定手順](uptime-kuma-webhook-setup.md) を参照してこと．

### 設定手順 (1) Notify Hub 自身の監視

まず，Uptime Kuma に Notify Hub 自体の監視 (HTTP) を追加することを推奨する．
- **監視URL例**: `http://<あなたのラズベリーパイのIPv4>:8090/health`
- **Method**: GET
...
- **URL**: 
  ...
  - LAN IP経由で送る場合: `http://<あなたのラズベリーパイのIPv4>:8090/api/events`
- **Method**: `POST`
...
- **Custom Headers**:
  ...
  ```
  ※ `.env` の `NOTIFY_API_TOKEN` と一致させてこと．

> **注意:** ルータのポート開放は絶対にしないこと．
...
固定値版が HTTP 201 で成功してから，以下の「変数版」へ進む流れを推奨する．

### Webhook JSON テンプレート（変数版）

以下のテンプレートは Uptime Kuma の変数を活用する例である．

> **【重要】Webhookテンプレート変数についての注意**
> 上記の `{{ }}` は Uptime Kuma のテンプレート変数である．
...
- [ ] Test 後に `http://<あなたのラズベリーパイのIPv4>:8090/events` を開き，`source=uptime-kuma` のイベントが増えていることを確認する
- [ ] 必要なら以下で確認する：
...
Windows 上での開発後，以下の手順でラズパイ上で動作確認を行ってこと．
...
**対処法:**
Uptime Kuma の Webhook ヘッダを見直してこと．

### 422 Unprocessable Entity が返る

**原因:**
JSON形式は届いているが，，Notify Hub の入力バリデーションに失敗している．
